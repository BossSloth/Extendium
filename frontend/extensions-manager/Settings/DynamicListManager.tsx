...
export type ExternalLinksManagerProps = {
  value: ExternalLink[];
  ...
  className?: string;
  allowEmpty?: boolean; // if false, empty items are filtered out on blur/remove
};
...
export default function DynamicListManager({
  ...
  className,
  allowEmpty = false,
}: ExternalLinksManagerProps): JSX.Element {
  const items = useMemo(() => value ?? [], [value]);
  const isFirstLoad = useRef(true);

  const setItem = useCallback(
    (index: number, next: ExternalLink, doSave: boolean = false) => {
